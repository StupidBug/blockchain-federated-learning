...
do
  python ../client.py --miner=127.0.0.1:5000 --dataset_dir=${dataset_dir} --epochs=5 --common_rounds=500 --name="node_${i}" --updates_dir=${updates_dir} --learning_rate=${learning_rate} --dataset_type=${dataset_type} --train_batch_size=${train_batch_size}&
done